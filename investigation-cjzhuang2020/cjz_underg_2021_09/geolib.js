...
function getManhattanByGeohash(geohash1, geohash2) {//geohash�����
	var vector = getVector(geohash1, geohash2);
	var ans = Math.abs(vector[0]) + Math.abs(vector[1]);
	return ans;
}
...
function getVector(geohash1, geohash2) { //geohash������
	...
	console.log("getLonDelta: ",londelta)
	...
	ans.push(dislon);
	ans.push(dislat);
	console.log("londelta latdelta dislat dislon", londelta, latdelta, dislat, dislon, dislat2 - dislat1, dislon2 - dislon1)
	return ans;
}
...
function getLonDelta(geohash) {//��ǰ�������
	...
	console.log("5-lat: ",lat);
	if ((lat & (1 << divnum)) != (1 << divnum)) {
		lat = (1 << (divnum + 1) - 1) - lat;
	}
	lat = lat - (1 << divnum);
	console.log("index-lat: ",lat)
	return deltaLon[lat];
}
